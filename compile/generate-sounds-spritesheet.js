...
export default async function generateSoundSprites (root) {
  ...
  return new Promise((resolve, reject) => {
    const output = path.resolve(OUTPUT_DIR, 'sounds/common')
    const options = {
      gap: 0.5,
      output
    }
    ...
    audiosprite(files, options, (err, generated) => {
      if (err) {
        return reject(err)
      }
...
// 	const spacer = await mp3Duration(silenceBuffer);
// 	console.log('silence', spacer);
